...
export function AccountDataProvider({children,client=supabaseClient,manager:provided}:Props){
  ...
  useEffect(()=>{const run=++generation.current;const manager=managerRef.current;void(async()=>{await manager?.deactivate();if(run!==generation.current)return;if(auth.phase!=="signed-in"||!userId){setState({phase:"signed-out",services:null,error:null,conflictDomains:[]});return;}if(!manager)throw new Error("Authenticated PowerSync is unavailable");setState({phase:"checking",services:null,error:null,conflictDomains:[]});const session=await manager.activate(userId);const binding=await readAccountBinding(session.database,userId);if(binding)await clearAdoptionJournal(session.database,userId);if(run!==generation.current)return;if(!binding){await manager.deactivate();if(run===generation.current)setState({phase:"needs-choice",services:null,error:null,conflictDomains:[]});return;}const services=createAuthenticatedDataServices(session.database,userId,undefined,session.registerCleanup);await snapshotData(services);if(run===generation.current)setState({phase:"ready",services,error:null,conflictDomains:[]});})().catch(error=>{if(run===generation.current)setState({phase:"error",services:null,error:error instanceof Error?error.message:String(error),conflictDomains:[]});});return()=>{generation.current+=1;void manager?.deactivate();};},[auth.phase,userId]);
