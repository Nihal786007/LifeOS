...
export const DEFAULT_QWEN_ATLAS_MODEL = "qwen3.7-plus-2026-05-26";
export const DEFAULT_QWEN_API_BASE_URL =
  "https://dashscope-intl.aliyuncs.com/compatible-mode/v1";

type FetchLike = (input: string | URL | Request, init?: RequestInit) => Promise<Response>;

export interface QwenAtlasAdapterOptions {
  apiKey: string;
  model?: string;
  apiBaseUrl?: string;
  fetchImpl?: FetchLike;
...
function validatedBaseUrl(value: string): string {
  ...
  if (parsed.protocol !== "https:" || parsed.username || parsed.password) {
    throw new Error("Qwen API base URL must be a credential-free HTTPS URL.");
  }
...
function parseModelOutput(text: string): Omit<HostedModelAdapterResult, "inputTokens" | "outputTokens"> {
  ...
  try {
    ...
    throw new Error("Qwen returned malformed structured output.");
  }
  if (!isRecord(parsed) || !hasExactKeys(parsed, [
    ...
    throw new Error("Qwen returned an invalid ATLAS output shape.");
  }
  return {
...
function responseText(value: unknown): { text: string; usage: unknown } {
  if (!isRecord(value) || !Array.isArray(value.choices) || value.choices.length !== 1) {
    throw new Error("Qwen returned no single ATLAS choice.");
  }
  ...
  if (!isRecord(choice) || choice.finish_reason !== "stop" ||
    ...
    throw new Error("Qwen returned incomplete, empty, or tool-bearing ATLAS output.");
  }
  return { text: choice.message.content, usage: value.usage };
}

export function buildQwenAtlasRequest(request: HostedAtlasRequest, model: string) {
  ...
  return {
    ...
    messages: [
      {
        role: "system",
        content: `${prompt.system}\nReturn one JSON object with exactly commentary, factReferences, and limitations.`,
      },
      {
        role: "user",
...
export function createQwenAtlasAdapter(options: QwenAtlasAdapterOptions): HostedModelAdapter {
  const apiKey = options.apiKey.trim();
  if (!apiKey) throw new Error("Qwen API key is required.");
  const model = options.model?.trim() || DEFAULT_QWEN_ATLAS_MODEL;
  const apiBaseUrl = validatedBaseUrl(
    options.apiBaseUrl?.trim() || DEFAULT_QWEN_API_BASE_URL
  );
  ...
  return {
    ...
    async generate(request, { signal }) {
      const response = await fetchImpl(`${apiBaseUrl}/chat/completions`, {
        ...
        body: JSON.stringify(buildQwenAtlasRequest(request, model)),
      });
      if (!response.ok) {
